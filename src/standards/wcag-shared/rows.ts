/**
 * Shared WCAG success criteria rows common to WCAG 2.1 and 2.2.
 *
 * These 78 rows are identical between the two versions (the normative
 * wording did not change for any pre-existing criterion). Each
 * version-specific module imports this shared set and adds its own
 * overrides (e.g., 4.1.1 Parsing differs between 2.1 and 2.2) plus
 * any version-specific criteria.
 *
 * Thin standards (Section 508, EN 301 549) also import from here
 * instead of depending on a specific WCAG version module, which
 * avoids the counter-intuitive "2.1 derived from 2.2" relationship.
 *
 ...
 *
 * `equivalentTo` entries cross-map to WCAG 2.1 from the WCAG 2.2
 * perspective. Version-specific modules may ignore or override this
 * field when building their own criterion records.
 */

import type { Automatability } from "../../types/standard.ts";

/**
 * Row shape used to build criteria lists. Exported so all WCAG-derived
 * standard modules can share this structure without duplication.
 */
...
/** The 78 WCAG rows shared between WCAG 2.1 and WCAG 2.2. */
export const SHARED_WCAG_ROWS: readonly WcagRow[] = [
  // =========================================================================
