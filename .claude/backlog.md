...
- [ ] **P0-F** Apply the existing `{error, code, details, remediation}` envelope (used today by `baseline check` without a file, `apply_fix` on multi-match, `scan_diff` on missing baseline) to silent-failure paths: nonexistent `cwd` on `scan_project` → `code: "cwd_not_found"` (or `"scan_root_empty"`); `scan_file` on non-parseable path → existing pattern but consistent. Pairs with P0-E's warnings array — the warning approach handles soft signals (Tailwind undercount, no_config_found); the error envelope handles hard malformed-input cases. Critical-severity, unanimous in R2.
- [ ] **P1-CHG** `changedOnly: true` silently falls back to a full scan when there are no staged files. Currently the response reports `scanMode: "changedOnly"` AND `filesScanned: 222` — the field lies about what ran. Two acceptable shapes: (a) `code: "no_staged_files"` error envelope, or (b) `scanMode: "full-fallback"` with `fallbackReason: "no-staged-files"`. Critical-severity per the brief; pre-commit / CI-on-diff workflows cannot detect their diff gate is no-op today.
- [ ] **P1-CFG** Bogus-`cwd` response also omits `configSource` / `rootSource` / `rulesEvaluated`, so an agent doing schema-presence checks gets a *different* shape than a healthy clean scan. Fix by always populating those fields with explicit "absent" values (e.g., `configSource: null` rather than missing). Compounds P1-CWD's silent-failure mode.
...
- [ ] **P1-DUP** Dedupe `reviewPrompt` text in `review_candidates`. Today every candidate carries the same ~450-char prompt verbatim; for a criterion with 15 candidates that's ~6 KB of duplicated text. Emit `prompts: { [criterionId]: { text, ...meta } }` once at the top level; each candidate references its criterion ID. Lossless transform; consumers reading per-candidate just look up.
- [ ] **P1-FILE-ENV** `scan_file` returns a stripped response envelope: only `{findings, reviewCandidates}`, no `meta.configSource`, no `activeNativeWrappers`, no `nextStep`, no `opaqueCustomComponents`. The fix-verify loop is the stated use case for `scan_file`, so the agent has no way to confirm config parity with the originating `scan_project`. Match `scan_project`'s envelope exactly.
- [ ] **P1-FILE-DEDUP** `scan_file` emits the same candidate under all four loaded standards (`wcag22`, `wcag21`, `section508`, `en301549`); `scan_project` correctly dedupes to a single finding with `criteria: [...]`. Mirror the `scan_project` dedup pass in `scan_file`.
- [ ] **P1-ACCT** `opaqueCustomComponents` count flips between calls in the same session (43 with `autoDetectWrappers: true`, 51 on a follow-up `changedOnly: true` call without the flag). The flag is scan-scoped, but `meta.activeNativeWrappers` doesn't make that obvious. Either persist `autoDetectWrappers` for the session once set (changes semantics — tighten docs) or surface the flag's scope in `meta.activeNativeWrappersBySource` so the count change is explainable.
- [ ] **P1-IGN** Subpath scans bypass root `.gitignore`. `scan_project({ cwd: "<repo>" })` honors `<repo>/.gitignore`; `scan_project({ cwd: "<repo>/<subdir>" })` does not, because the discovery walker starts beneath the gitignore root. Result: same tree, different violation counts depending on `cwd` depth — agents comparing before/after across `cd` boundaries mis-attribute regressions. Walk up to the git root to collect gitignore semantics, then scope file enumeration to `cwd`.
- [ ] **P1-CONF** Typed `confidence: "high" | "medium" | "low"` on every manual-review candidate (today the prose says "low confidence" but it's not a structured field). Lets agents threshold and sort. Pairs with the existing `confidence` field already on automated findings — same enum, same semantics.
