...
- [ ] **Q2-WRAPMAP-RULES** Wire `LoadedConfig.nativeWrapperElements` through rule-execution so rules that depend on the native element (link-descriptive-text, alt-text, forms-required) run through mapped wrappers as if they were the native tag. Q2-WRAPMAP landed the config shape; this item consumes it. Per-rule opt-in so non-element-dependent rules keep ignoring the map.
