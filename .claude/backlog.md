...
- [~] **Q2R2-COVERAGE-CHECKLIST** ADR 0010 landed (fe962f6) deciding keep-both for v0.2.0 + sharpen boundary: trim `checklist.summary.automatedCoverage` to a one-field gloss and add cross-pointing `nextStep`+`nextStepStructured` on both tools. Merge with verbosity knob deferred to v1.0.0 behind a deprecation alias path. Implementation pending (trim summary shape, wire nextStep, add cross-tool consistency test). [round 2, agent 7]
...
- [~] **Q2R2-POLYMORPHIC** Resolve literal `as` / `asChild` prop values and re-dispatch rules against the resolved element. `<Button as="a" href=...>` re-runs `link-no-href`, `link-descriptive-text`. Only literals — dynamic `as={Something}` stays unresolved (honest — agent reads and decides). Engine surface shipped (0b130dd: `ctx.resolvePolymorphic`, link-descriptive-text opted in); follow-up opt-ins pending for alt-text-missing, labels-required, button-name. [round 2, agent 2]
...
- [~] **Q2R2-CWBB** Shape + forwarders landed (2df787a + ADR 0009). Optional `readonly couldBeWrongBecause?: readonly string[]` on `Violation`; wired through MCP `formatFinding` and the `agent` formatter with conditional-spread so empty/absent reads identically to consumers. `json` + SARIF forwarders deferred to the first rule opt-in commit. Per-rule opt-in pending — first candidates: `forms/required-indicator-missing` → `replacement_indicator_in_sibling_file`; contrast rules → `tailwind_class_on_consumer`. `estimatedFpRate` numeric remains rejected per §1 "Numeric-threshold heuristics are suppression." [round 2, agent 8]
