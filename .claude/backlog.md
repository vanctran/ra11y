...
- [ ] **Q2-WRAPMAP-MCP** Thread `LoadedConfig.nativeWrapperElements` through the MCP handlers so `configure` / `sessionConfigure` / `scan` / `scan_project` surface the element mapping alongside the flat names list. Today `src/mcp/` imports `nativeWrappers` names but not `nativeWrapperElements`; agents can't round-trip the object form via MCP. Additive; no shape break.
